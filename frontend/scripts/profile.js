const baseURL = "http://localhost:8000"; // declared constant

async function loadUsers() {
  const res = await fetch(`${baseURL}/users`); // added baseurl/users path
  const users = await res.json();
  const list = document.getElementById("userList");
  list.innerHTML = "";
  // userCounts as according to HTML
...
document.getElementById("search").addEventListener("input", async (e) => {
  ...
  filteredUsers.forEach(user => {
    ...
    deleteBtn.textContent = "Delete";
    deleteBtn.onclick = async () => {
      await fetch(`${baseURL}/users/${user._id}`, { method: "DELETE" }); // changed patch to delete
      loadUsers();
    };
